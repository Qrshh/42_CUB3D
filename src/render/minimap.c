...
/*   Updated: 2025/02/12 16:37:29 by abesneux         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_minimap(t_all *all)
{
    ...
    for (map_y = 0; all->map[map_y]; map_y++)
    {
        ...
        {
            int color = (all->map[map_y][map_x] == '1') ? 0xFFFFFF : 0x00FF3A;
            for (int i = 0; i < tile_size; i++)
            {
                for (int j = 0; j < tile_size; j++)
                {
                    mlx_put_pixel(all->minimap_img, offset_x + map_x  * tile_size + i, offset_y + map_y * tile_size + j, color);
                }
            }
        }
    }
    // Dessiner le joueur en rouge
    int player_x = offset_x + (all->player_pos.x / TILE_SIZE) * tile_size;
    int player_y = offset_y + (all->player_pos.y / TILE_SIZE) * tile_size;
    for (int i = 0; i < tile_size / 2; i++)
    {
        for (int j = 0; j < tile_size / 2; j++)
        {
            mlx_put_pixel(all->minimap_img, player_x + i, player_y + j, 0xFF49FF);
        }
    }
